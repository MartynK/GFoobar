...
# Functions for debug and eval.

from random import randint

def randomlist(x,y):
	out = [[randint(1,10000) for i in range(x)] for j in range(y)]
	return out
...
############
# Main function. Has no dependencies.
	
def answer(list,x,y,strength):
	neighbours = [[x,y-1],[x,y+1],[x-1,y],[x+1,y]]
	for i in neighbours:
		x_act = i[0]
		y_act = i[1]
		try:
			if list[x_act][y_act] <= strength:
				if list[x_act][y_act] != -1:
					list[x_act][y_act] = -1
					answer(list,x_act,y_act,strength)
		except IndexError:
			pass
...
nicelistprint(answer(randomlist(4,7),1,2,6500))